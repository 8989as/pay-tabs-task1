...
const CategorySelection = () => {
  ...
  const onAddCategoryHandler = () => {
    console.log("Current CatChoices:", CatChoices); // Debugging log
    if (CatChoices.category !== "") {
      const existItem = choosedCategories.find(
        (item) => item.Categoryid === parseInt(CatChoices.category)
      );
      console.log("Exist item check:", existItem); // Debugging log

      if (!existItem) {
        const selectedCategory = webCategories.find(
          (category) => category.CategoryID === parseInt(CatChoices.category)
        );
            ...
            <div className="col-12 row justify-content-between g-4 mb-3">
              ...
              <div className="col-4">
                ...
                >
                  ...
                  {webCategories.map((option, index) =>
                    option.ParentCategoryID === parseInt(CatChoices.header) ? (
                      <option key={index} value={option.CategoryID}>
                        {option.EnglishName}
                      </option>
                    ) : null
              ...
              <div className="col-4">
                ...
                <select
                  ...
                  name="subCategory"
                  className="form-control"
                  value={CatChoices.subCategory}
                  onChange={categoryOnChangeHandler}
                >
                  <option value="">Select SubCategory</option>
                  {webCategories.map((option, index) =>
                    option.ParentCategoryID === parseInt(CatChoices.category) ? (
                      <option key={index} value={option.CategoryID}>
                        {option.EnglishName}
                      </option>
                    ) : null
